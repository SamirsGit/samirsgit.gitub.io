...
**cat ./-** - recently, cat was introduced as a command that displays the content of a file, the only one shown here is a dash. We need to indicate that it's a file in the current directory, not an option, which is why we prefix the filename with ./ -- The creator of the game tries to trick us, files that start with a dash can cause problems because they're often interpretted as the beginning of an option, (for example: ls -a). 
